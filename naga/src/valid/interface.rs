...
pub enum VaryingError {
    ...
    #[error(
        "The `blend_src` attribute can only be used on location 0, only indices 0 and 1 are valid. Location was {location}, index was {blend_src}."
    )]
    InvalidBlendSrcIndex { location: u32, blend_src: u32 },
    #[error(
        "If `blend_src` is used, there must be exactly two outputs both with location 0, one with `blend_src(0)` and the other with `blend_src(1)`."
    )]
    IncompleteBlendSrcUsage,
    #[error(
        "If `blend_src` is used, both outputs must have the same type. `blend_src(0)` has type {blend_src_0_type:?} and `blend_src(1)` has type {blend_src_1_type:?}."
    )]
    ...
    #[error(
        "Workgroup size is multi dimensional, `@builtin(subgroup_id)` and `@builtin(subgroup_invocation_id)` are not supported."
    )]

...
use super::{Error, LocationMode, Options, PipelineOptions, TranslationInfo, sampler as sm};
use crate::{
    FastHashMap, FastHashSet,
    arena::{Handle, HandleSet},
    back::{self, Baked, get_entry_points},
    common,
    proc::{
        self, NameKey, TypeResolution,
...
{level}{loop_bound_name} -= uint2({loop_bound_name}.y == 0u, 1u);"
                ...
                crate::Statement::RayQuery { query, ref fun } => {
                    ...
                    match *fun {
                        crate::RayQueryFunction::Initialize {
                            ...
                            writeln!(
                                self.out,
                                ".{RAY_QUERY_FIELD_INTERSECTOR}.assume_geometry_type({RT_NAMESPACE}::geometry_type::triangle);"
                            )?;
                            {
                                ...
                                write!(
                                    self.out,
                                    ".flags & {f_opaque}) != 0 ? {RT_NAMESPACE}::opacity_cull_mode::opaque : ("
                                )?;
                                self.put_expression(descriptor, &context.expression, true)?;
                                write!(
                                    self.out,
                                    ".flags & {f_no_opaque}) != 0 ? {RT_NAMESPACE}::opacity_cull_mode::non_opaque : "
                                )?;
                            ...
                            }
                            {
                                ...
                                write!(
                                    self.out,
                                    ".flags & {f_opaque}) != 0 ? {RT_NAMESPACE}::forced_opacity::opaque : ("
                                )?;
                                self.put_expression(descriptor, &context.expression, true)?;
                                write!(
                                    self.out,
                                    ".flags & {f_no_opaque}) != 0 ? {RT_NAMESPACE}::forced_opacity::non_opaque : "
                                )?;
    ...
    fn put_ray_query_type(&mut self) -> BackendResult {
        ...
        let full_type = format!(
            "{RT_NAMESPACE}::intersector<{RT_NAMESPACE}::instancing, {RT_NAMESPACE}::triangle_data, {RT_NAMESPACE}::world_space_data>"
        );
        ...
        writeln!(
            self.out,
            "constexpr {NAMESPACE}::uint {RAY_QUERY_FUN_MAP_INTERSECTION}(const {RT_NAMESPACE}::intersection_type ty) {{"
        )?;
        ...
        for (expr_handle, expr) in func_ctx.expressions.iter() {
            match *expr {
                ...
                crate::Expression::Math {
                    ...
                    match fun {
                            ...
                            if arg_ty.scalar_kind() == Some(crate::ScalarKind::Sint) =>
                        {
                            ...
                            writeln!(
                                self.out,
                                "{level}return metal::select(as_type<{type_name}>(-as_type<{unsigned_type_name}>(val)), val, val >= 0);"
                            )?;
                            writeln!(self.out, "}}")?;
                            writeln!(self.out)?;
                        }
                ...
                crate::Expression::ImageSample {
                    ...
                    writeln!(
                        self.out,
                        "{NAMESPACE}::float4 {IMAGE_SAMPLE_BASE_CLAMP_TO_EDGE_FUNCTION}({NAMESPACE}::texture2d<float, {NAMESPACE}::access::sample> tex, {NAMESPACE}::sampler samp, {NAMESPACE}::float2 coords) {{"
                    )?;
                    let l1 = back::Level(1);
                    writeln!(
                        self.out,
                        "{l1}{NAMESPACE}::float2 half_texel = 0.5 / {NAMESPACE}::float2(tex.get_width(0u), tex.get_height(0u));"
                    )?;
                    writeln!(
                        self.out,

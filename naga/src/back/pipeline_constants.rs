...
pub fn process_overrides<'a>(
    ...
    let global_expressions_to_process: alloc::vec::Vec<_> =
        module.global_expressions.drain().collect();
    for (old_h, expr, span) in global_expressions_to_process {
        let mut expr = {
            // First handle override and constant processing with its own scope
            match expr {
        ...
        }; // End scope to drop all mutable borrows of module

        // Now create evaluator in a fresh scope

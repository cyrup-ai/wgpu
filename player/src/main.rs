...
fn main() {
    ...
    let path = if args.len() > 1 {
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/data/all.ron")
    };

    Corpus::run_from(path);
}
...
fn test_api() {
    ...
    Corpus::run_from(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/data/all.ron"))
}

...
fn wasm32_without_webgl_or_noop_does_not_depend_on_wgpu_core() {
    ...
    check_feature_dependency(Requirement {
        human_readable_name: "wasm32 without `webgl` or `noop` feature does not depend on `wgpu-core`",
...
fn wasm32_with_only_custom_backend_does_not_depend_on_web_specifics() {
    check_feature_dependency(Requirement {
        ...
        default_features: false,
        search_terms: &[
            Search::Negative("wasm-bindgen"),
            Search::Negative("js-sys"),
            Search::Negative("web-sys"),
        ],
    });
}
...
fn wasm32_with_webgpu_backend_does_depend_on_web_specifics() {
    check_feature_dependency(Requirement {
        ...
        default_features: false,
        search_terms: &[
            Search::Positive("wasm-bindgen"),
            Search::Positive("js-sys"),
            Search::Positive("web-sys"),
        ],
    });
}
...
fn apple_with_vulkan_portability_depends_on_ash_and_renderdoc_sys() {
    check_feature_dependency(Requirement {
        human_readable_name: "apple with `vulkan-portability` feature depends on `ash` and `renderdoc-sys`",
        target: "aarch64-apple-darwin",
        packages: &["wgpu"],
...
fn windows_with_no_features_does_not_depend_on_glow_windows_or_ash() {
    check_feature_dependency(Requirement {
        human_readable_name: "windows with no features does not depend on 'glow', `windows`, or `ash`",
        target: "x86_64-pc-windows-msvc",
        packages: &["wgpu"],
        features: &[],
        default_features: false,

...
use wgpu_test::{GpuTestConfiguration, TestParameters, TestingContext, gpu_test};
...
async fn fill_test(ctx: &TestingContext, range: Range<u64>, size: u64) -> bool {
    ...
    if second_clear_byte.is_some() {
        eprintln!(
            "Found multiple cleared ranges instead of a single clear range of {}..{} on a buffer of size {}.",

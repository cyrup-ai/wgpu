...
use wgpu_test::{GpuTestConfiguration, TestParameters, TestingContext, fail_if, gpu_test};
...
async fn map_test(
    ...
    log::info!(
        "map_test usage_type:{usage_type} map_mode_type:{:?} before_unmap:{before_unmap} before_destroy:{before_destroy} after_unmap:{after_unmap} after_destroy:{after_destroy}",
        map_mode_type
    );

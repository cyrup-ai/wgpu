...
use wgpu_test::{
    FailureCase, GpuTestConfiguration, TestParameters, TestingContext, fail, gpu_test,
};
...
fn encoder_operations_fail_while_pass_alive(ctx: TestingContext) {
    ...
    for &pass_type in [PassType::Compute, PassType::Render].iter() {
        for (op_name, op) in recording_ops.iter() {
            ...
            log::info!(
                "Testing operation {op_name:?} on a locked command encoder while a {pass_type:?} pass is active"
            );
            op(&mut encoder);

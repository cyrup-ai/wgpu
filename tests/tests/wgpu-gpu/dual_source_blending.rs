use wgpu::*;
use wgpu_test::{GpuTestConfiguration, TestParameters, TestingContext, fail, gpu_test};
...
async fn dual_source_blending_disabled(ctx: TestingContext) {
    ...
    fail(
        ...
        Some(
            "Features Features { features_wgpu: FeaturesWGPU(0x0), features_webgpu: FeaturesWebGPU(DUAL_SOURCE_BLENDING) } are required but not enabled on the device",
        ),
    );

...
fn external_texture_binding_texture_view() {
    ...
    fail(
        ...
        Some(
            "Usage flags TextureUsages(STORAGE_BINDING) of TextureView with '' label do not contain required usage flags TextureUsages(TEXTURE_BINDING",
        ),
    );
    ...
    fail(
        ...
        Some(
            "External texture bindings must have a single mip level, but given a view with mip_level_count = 2 at binding 0",
        ),
    );
    ...
    fail(
        ...
        Some(
            "External texture bindings must have a format of `rgba8unorm`, `bgra8unorm`, or `rgba16float, but given a view with format = Rgba8Uint at binding 0",
        ),
    );

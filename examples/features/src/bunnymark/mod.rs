...
// SAFETY: Bunny is a simple struct with all fields being Pod/Zeroable compatible
// The large padding array [u8; 236] consists only of u8 values which are Pod/Zeroable
unsafe impl bytemuck::Pod for Bunny {}
unsafe impl bytemuck::Zeroable for Bunny {}
    ...
    fn init(
        ...
        let texture = {
            let (size, buf) = {
                ...
                let img_data = &[]; // Empty data as placeholder
                let decoder = png::Decoder::new(std::io::Cursor::new(img_data));
                match decoder.read_info().and_then(|mut reader| {
                    let info = reader.info();
                    let buffer_size = reader
                        .output_buffer_size()
                        .unwrap_or(4 * info.width as usize * info.height as usize);
                    let mut buf = vec![0; buffer_size];
                    reader.next_frame(&mut buf).map(|frame_info| {
                        (
                            wgpu::Extent3d {
                    ...
                    Ok(result) => result,
                    Err(e) => {
                        log::warn!("Failed to decode embedded PNG: {}, using fallback", e);
                        // Create a simple 2x2 white texture as fallback
                        (
                            wgpu::Extent3d {
                                width: 2,
                                height: 2,
                            ...
                            vec![255u8; 16],
                        ) // 2x2 RGBA white pixels
                    }
                }
            };

            let texture = device.create_texture(&wgpu::TextureDescriptor {
                label: Some("bunnymark_texture"),
                size,
                mip_level_count: 1,
                sample_count: 1,

...
use crate::{Label, api_log, global::Global, id, resource_log};
use crate::{LabelHelpers, hal_label};
...
pub enum EncoderStateError {
    ...
    #[error(
        "Encoder is locked by a previously created render/compute pass. Before recording any new commands, the pass must be ended."
    )]
    Locked,
...
pub enum TimestampWritesError {
    ...
    #[error(
        "no begin or end indices were specified for pass timestamp writes, expected at least one to be set"
    )]
    IndicesMissing,
}

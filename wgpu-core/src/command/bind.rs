            ...
            if let Some(expected_bgl) = self.expected.as_ref() {
                if let Some(assigned_bgl) = self.assigned.as_ref() {
                    if expected_bgl.is_equal(assigned_bgl) {
                        ...
                        enum EntryError {
                            #[error(
                                "Entries with binding {binding} differ in visibility: expected {expected:?}, got {assigned:?}"
                            )]
                            Visibility {
                                binding: u32,
                            ...
                            #[error(
                                "Entries with binding {binding} differ in type: expected {expected:?}, got {assigned:?}"
                            )]
                            Type {
                                binding: u32,
                            ...
                            #[error(
                                "Entries with binding {binding} differ in count: expected {expected:?}, got {assigned:?}"
                            )]
                            Count {
                                binding: u32,
                            ...
                            #[error(
                                "Expected entry with binding {binding} not found in assigned bind group layout"
                            )]
                            ExtraExpected { binding: u32 },
                            #[error(
                                "Assigned entry with binding {binding} not found in expected bind group layout"
                            )]
...
pub enum BinderError {
    ...
    #[error(
        "The {assigned_bgl} of current set {assigned_bg} at index {index} is not compatible with the corresponding {expected_bgl} of {pipeline}"
    )]

...
use crate::{
    api_log,
    command::{CommandEncoderError, EncoderStateError, clear_texture},
    conv,
...
pub enum TransferError {
    ...
    #[error(
        "Copy of {start_offset}..{end_offset} would end up overrunning the bounds of the {side:?} buffer of size {buffer_size}"
    )]
    BufferOverrun {
    ...
    #[error(
        "Copy of {dimension:?} {start_offset}..{end_offset} would end up overrunning the bounds of the {side:?} texture of {dimension:?} size {texture_size}"
    )]
    TextureOverrun {
        start_offset: u32,
        end_offset: u32,
    ...
    #[error("Copy destination aspects must refer to all aspects of the destination texture format")]

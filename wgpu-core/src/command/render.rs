...
use crate::command::{
    EncoderStateError, PassStateError, TimestampWritesError, pass, pass_base, pass_try,
...
use crate::{
    ...
    command::{
        ...
        memory_init::{SurfacesInDiscardState, fixup_discarded_surfaces},
    },
    device::{
...
use super::{
    CommandBufferTextureMemoryActions, CommandEncoder, QueryResetMap,
    memory_init::TextureSurfaceDiscard,
};
...
pub enum ColorAttachmentError {
    ...
    #[error(
        "The total number of bytes per sample in color attachments {total} exceeds the limit {limit}"
    )]
    ...
    UnneededDepthSlice,
    #[error(
        "{view}'s subresource at mip {mip_level} and depth/array layer {depth_or_array_layer} is already attached to this render pass"
    )]
    SubresourceOverlap {
...
pub enum RenderPassErrorInner {
    ...
    #[error(
        "Attachments have differing sizes: the {expected_location} has extent {expected_extent:?} but is followed by the {actual_location} which has {actual_extent:?}"
    )]
    ...
    #[error(
        "Attachments have differing sample counts: the {expected_location} has count {expected_samples:?} but is followed by the {actual_location} which has count {actual_samples:?}"
    )]
    ...
    #[error(
        "The resolve source, {location}, must be multi-sampled (has {src} samples) while the resolve destination must not be multisampled (has {dst} samples)"
    )]
    ...
    #[error(
        "Indirect draw uses bytes {offset}..{end_offset} using count {count} which overruns indirect buffer of size {buffer_size}"
    )]
    ...
    #[error(
        "Indirect draw uses bytes {begin_count_offset}..{end_count_offset} which overruns indirect buffer of size {count_buffer_size}"
    )]
...
fn draw_indexed(
    ...
    api_log!(
        "RenderPass::draw_indexed {index_count} {instance_count} {first_index} {base_vertex} {first_instance}"
    );

...
pub(crate) use texture::{
    TextureInitRange, TextureInitTracker, TextureInitTrackerAction,
    ...
    Idx: fmt::Debug + Ord + Copy + Default,
{
    ...
    pub(crate) fn uninitialized<'a>(
        &'a mut self,
        drain_range: Range<Idx>,
    ) -> UninitializedIter<'a, Idx> {
        let index = self
            .uninitialized_ranges
    ...
    pub(crate) fn drain<'a>(&'a mut self, drain_range: Range<Idx>) -> InitTrackerDrain<'a, Idx> {
        let index = self
            .uninitialized_ranges
            .partition_point(|r| r.end <= drain_range.start);
        InitTrackerDrain {

...
pub enum CreateBlasError {
    ...
    MissingIndexData,
    #[error(
        "Provided format was not within allowed formats. Provided format: {0:?}. Allowed formats: {1:?}"
    )]
...
pub enum BuildAccelerationStructureError {
    ...
    #[error("Blas {0:?} build sizes to may be greater than the descriptor at build time specified")]
    ...
    #[error(
        "Blas {0:?} build vertex count is greater than creation count (needs to be less than or equal to), creation: {1:?}, build: {2:?}"
    )]
    ...
    #[error(
        "Blas {0:?} build index count is greater than creation count (needs to be less than or equal to), creation: {1:?}, build: {2:?}"
    )]
...
pub enum BlasPrepareCompactError {
    ...
    NotBuilt,
    #[error(
        "BLAS does not support compaction (is AccelerationStructureFlags::ALLOW_COMPACTION missing?)"
    )]

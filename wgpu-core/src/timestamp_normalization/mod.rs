...
impl TimestampNormalizer {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            {
                log::error!(
                    "Automatic timestamp normalization was requested, but compute shaders are not supported."
                );
                return Ok(Self { state: None });
            }
    ...
    pub unsafe fn create_normalization_bind_group(
        ...
        unsafe {
            let state = match &self.state {
                Some(state) => state,
                None => return Ok(TimestampNormalizationBindGroup { raw: None }),
            };
    ...
    pub fn normalize(
        ...
        let bind_group = match bind_group.raw.as_deref() {

    ...
    impl ApplicationHandler for App {
        fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            if self.window.is_none() {
                let window_attrs =
                    winit::window::WindowAttributes::default().with_title("hal-bunnymark");
                let window = event_loop.create_window(window_attrs).unwrap();
                let example_result = Example::<Api>::init(&window);
                let example = example_result.expect("Selected backend is not supported");
        ...
        fn window_event(
            ...
            match event {
                ...
                WindowEvent::RedrawRequested => {
                    if let (Some(example), Some(window)) = (&mut self.example, &self.window) {
                        {
                            self.accum_time += self.last_frame_inst.elapsed().as_secs_f32();
                            self.last_frame_inst = Instant::now();
                            self.frame_count += 1;
                            if self.frame_count == 100 && !example.is_empty() {
                                println!(
                                    "Avg frame time {}ms",
                ...
                _ => {
                    if let Some(ref mut example) = self.example {
